...
GxE=do.call(rbind,lapply(files_general,function(f){
  ...
  anc_var = ( df_anc %>% summarize(Va=sum(anc_effect_size^2*2*freq_A*(1-freq_A))))$Va
  ...
  do.call("rbind", lapply(df, function(df_g){
    ...
    sub_df_g=df_g %>% filter(freq_A>0 & popA_effect_size!=0) %>% mutate(var_popA = popA_effect_size^2*2*freq_A*(1-freq_A))
    ...
    get_numerator=function(mut_ID,freq,effect_size_A,effect_size_B){
      data.frame(t(sapply(ascertainment,function(ascertained_mut){
        keep=mut_ID %in% ascertained_mut
        sum(2*effect_size_A[keep]*effect_size_B[keep]*freq[keep]*(1-freq[keep]))^2 / # numerator is squared covar, denom is var of
          sum(2*effect_size_A[keep]^2*freq[keep]*(1-freq[keep]))
      })))
    ...
    space =df_g %>% group_by(sim_type,w,sd_a,corr,nTraits,opt_shift, opt_shift_start_gen ,run,generation) %>%
      group_modify(~ get_numerator(.x$mut_ID,.x$freq_B,.x$popA_effect_size,.x$popB_effect_size))

    space = space %>% pivot_longer(cols=all_of(ascertainment_cols),
                                   names_to="ascertainment",
                                   values_to="cov")
    var = sum(df_g$popB_effect_size^2*2*df_g$freq_B*(1-df_g$freq_B))
    ...
    GWAS_pop =df_g %>% group_by(sim_type,w,sd_a,corr,nTraits,opt_shift, opt_shift_start_gen ,run,generation) %>%
      group_modify(~ get_numerator(.x$mut_ID,.x$freq_A,.x$popA_effect_size))
    GWAS_pop = GWAS_pop %>% pivot_longer(cols=all_of(ascertainment_cols),
                                   names_to="ascertainment",
                                   values_to="cov")
    var_A = sum(df_g$popA_effect_size^2*2*df_g$freq_A*(1-df_g$freq_A))
    ...
    time=data.frame(t(sapply(ascertainment,function(ascertained_mut){
      (df_anc %>% ungroup() %>% filter(mut_ID %in% ascertained_mut) %>%
         summarise(cov=sum(anc_effect_size*popA_effect_size*2*freq_A*(1-freq_A)),
                   p_var=sum(popA_effect_size^2*2*freq_A*(1-freq_A))) %>%
         mutate(part=cov^2 / p_var))$part }))) %>%
