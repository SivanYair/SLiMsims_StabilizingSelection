...
input_path="path/Pleiotropy/divergence_output/mut_track/"
...
Pleiotropy=do.call(rbind,lapply(files_general,function(f){

  # read lines of output; each line becomes a character element in the vector
  ...
  corr=1
  nTraits=strsplit(f,split="_")[[1]][3]
  nTraits=as.numeric(gsub("nTraits","",nTraits))
  ...
  do.call("rbind",lapply(d,function(df){
    ...
    ascertainment = list(
      all = df_sel$mut_ID,
      popA_maf.01 = df_sel$mut_ID[df_sel$freq_A>=0.01 & df_sel$freq_A<=0.99],
      popA_maf.001 = df_sel$mut_ID[df_sel$freq_A>=0.001 & df_sel$freq_A<=0.999]
    )
    # add ascertainment by variance contributed to ascertainment list
    sub_df_sel=df_sel %>% filter(freq_A>0 ) %>% mutate(var_popA = a1_effect_size^2*2*freq_A*(1-freq_A))
    ...
    # for each ascertainment category, get Qst btwn pops A & B
    get_Qst=function(mut_ID,freq_A,freq_B,tot_freq,effect_size){
      tmp=data.frame(t(sapply(ascertainment,function(ascertained_mut){
    ...
    Q = df %>% group_by(sim_type,w,sd_a,corr,nTraits,opt_shift,opt_shift_start_gen,run,generation) %>%
      group_modify(~ get_Qst(.x$mut_ID,.x$freq_A,.x$freq_B,.x$tot_freq,.x$a1_effect_size)) %>%
...
saveRDS(Pleiotropy,paste0(output_path,"Pleiotropy.RDS"))
