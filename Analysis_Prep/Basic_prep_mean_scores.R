...
input_path="input/Baseline/divergence_output/mut_track/"
...
Baseline=do.call(rbind,lapply(files_general,function(f){
  ...
  do.call("rbind",lapply(d,function(df){
    ...
    # return data frame where for each ascertainment category (column) the row is the value of mean score
...
saveRDS(Baseline,paste0(output_path,"Baseline.RDS"))
