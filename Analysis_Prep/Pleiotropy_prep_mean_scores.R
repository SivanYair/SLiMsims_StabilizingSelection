...
Pleiotropy=do.call(rbind,lapply(files_general,function(f){
  ...
  do.call("rbind",lapply(d,function(df){
    ...
    sub_df=df %>% filter(freq_A>0 ) %>% mutate(var_popA = a1_effect_size^2*2*freq_A*(1-freq_A))
    ...
    mean_A = df %>% group_by(sim_type,w,sd_a,corr,nTraits,opt_shift,opt_shift_start_gen,run,generation) %>%
      group_modify(~ get_score(.x$mut_ID,.x$freq_A,.x$a1_effect_size)) %>%
    ...
    mean_B = df %>% group_by(sim_type,w,sd_a,corr,nTraits,opt_shift,opt_shift_start_gen,run,generation) %>%
      group_modify(~ get_score(.x$mut_ID,.x$freq_B,.x$a1_effect_size)) %>%
