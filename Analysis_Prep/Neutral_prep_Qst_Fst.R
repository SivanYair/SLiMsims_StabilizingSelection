...
input_path="path/Neutral/divergence_output/mut_track/"
...
Neutral=do.call(rbind,lapply(files_general,function(f){
  #
  ...
  do.call("rbind",lapply(d,function(df){
    ...
    sub_df_sel=df_sel %>% filter(freq_A>0 ) %>% mutate(var_popA = effect_size^2*2*freq_A*(1-freq_A))
    ...
    Q = df %>% group_by(sim_type,w,sd_a,corr,nTraits,opt_shift,opt_shift_start_gen,run,generation) %>%
      group_modify(~ get_Qst(.x$mut_ID,.x$freq_A,.x$freq_B,.x$tot_freq,.x$effect_size)) %>%
...
saveRDS(Neutral,paste0(output_path,"Neutral.RDS"))
